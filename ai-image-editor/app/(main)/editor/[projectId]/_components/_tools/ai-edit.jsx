...
export function AIEdit({ project }) {
  ...
  const ensureImageKitUrl = async (imageUrl) => {
    ...
    try {
      ...
      formData.append("fileName", "ai-edit-upload.png");
  ...
  const applyRetouch = async () => {
    ...
    try {
      ...
      // Ensure image is hosted on ImageKit
      currentImageUrl = await ensureImageKitUrl(currentImageUrl);
      ...
      for (let i = 0; i < 20; i++) {
        ...
        if (i === 4) {
          ...
          setProcessingMessage("Still enhancing your image...");
        }
        
        try {
          const testResponse = await fetch(retouchedUrl, { method: 'HEAD' });
          if (testResponse.ok) {
            isReady = true;
            break;
          }
      ...
      if (!isReady) {
        throw new Error("AI processing failed or timed out. Please try again.");
      }

      // 4. Load the result back to Fabric.js with timeout
      const loadImagePromise = FabricImage.fromURL(retouchedUrl, {
      ...
      const retouchedImage = await Promise.race([loadImagePromise, timeoutPromise]);
